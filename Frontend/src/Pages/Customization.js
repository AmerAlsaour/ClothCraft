...
function Customization() {
  ...
  const exportDesign = async () => {
    ...
    const res = await fetch('http://localhost:5000/order/buy', {
      method: 'POST',
      credentials: 'include',
      ...
      <nav className="bg-white shadow-md sticky top-0 min-w-full z-10">
        <div className="container mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center">
            <button className=" bgblueden  text-xl  borderbluedent border-2 rounded-xl px-3 pb-1  text-white font-bold focus:outline-none focus:shadow-outline" onClick={handleClickSign}>
      ...
      <div className="flex flex-col md:flex-row p-4">
        ...
        <div className="w-full md:w-2/5 px-6 mt-10 mb-4 md:mb-0">
          ...
          <div className="mb-4">
            ...
            <div className="flex space-x-2">
              <button
                onClick={() => { setIsFront(true); setSelectedId(null) }}
                className={`px-4 py-2 rounded ${isFront ? 'bgblueden text-white' : 'bg-gray-200'}`}
              >
                Front
              </button>
              <button
                onClick={() => { setIsFront(false); setSelectedId(null) }}
                className={`px-4 py-2 rounded ${!isFront ? 'bgblueden text-white' : 'bg-gray-200'}`}
              >
                Back
              </button>
