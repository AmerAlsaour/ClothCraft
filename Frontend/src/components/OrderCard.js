...
const OrderCard = ({ front,back,buyer,location,id }) => {
...
<div class="flex pl-5 hover:cursor-pointer flex-col  items-center bg-stone-100 border border-gray-200 rounded-lg shadow md:flex-row md:w-[80%] hover:bg-stone-200 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 gap-4">
    ...
    <div className="flex flex-col p-4 leading-normal">
        <p className=" my-6  text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
          Buyer : {buyer}
        </p>
        <p className=" text-2xl my-6   tracking-tight text-gray-900 dark:text-white">
          Price : 50,000 s.p
        </p>
        <p className=" text-2xl my-6   tracking-tight text-gray-900 dark:text-white">
          Location : {location}
        </p>
        <NavLink className="focus:outline-none text-center text-white bg-orange-400 hover:bg-yellow-500 focus:ring-4 my-6  focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2  dark:focus:ring-yellow-900"
        to={id}>Details</NavLink>
    </div>  
</div>
